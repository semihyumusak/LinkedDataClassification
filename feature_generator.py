...
def Main(endpoint):
    ...
    with open('testDatasetLabeledSample.tsv','r') as f:
    ...
    from sklearn.feature_selection import SelectKBest, SelectPercentile,chi2,f_regression,f_classif,f_oneway,RFE,RFECV
    from sklearn.feature_selection import VarianceThreshold
    ...
    select = SelectPercentile(score_func=f_classif,percentile=1).fit(fit.transform(X).toarray(),y)
    fit.restrict (select.get_support())
    #support = SelectKBest(chi2,k=10).fit()
    ...
    exit()
    X_train_counts = fit.transform(X)# fit.transform(X)

    X_test_counts = fit.transform(featureListTest)# selector.transform(fit.transform(featureListTest)) # fit.transform(featureListTest)
